...
task import: :environment do
  ...
  agents.with_progress(desc: 'Creating Agents in Mongo').each do |agent|
    ...
    corp.agents.where(id: agent.agentID, name: agents_api.select { |a| a.id == agent.agentID }.first.name, division: divisions.select { |d| d['divisionID'] == agent.divisionID }.first['divisionName'], level: agent.level).first_or_create
  end
end
