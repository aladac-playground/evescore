...
class WalletRecord
  ...
  def self.create_from_api(character, wallet_record)
    record = new(
      amount: wallet_record.amount, tax: wallet_record.tax || 0, ts: wallet_record.date, date: wallet_record.date.to_date, user_id: character.user.id,
      type: wallet_record.ref_type, ref_id: wallet_record.ref_id, character_id: character.id, agent_id: wallet_record.first_party_id
    )
  ...
  def build_kills(text)
    return true if type != 'bounty_prizes'
    text.split(',').map { |a| a.split(":\s") }.each do |rat_info|
      kills.build(rat_id: rat_info[0].to_i, amount: rat_info[1], date: date, ts: ts, character_id: character_id, user_id: character.user.id)
    end
  end
end
